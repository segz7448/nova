 ...
 * - x402_domain_allowlist denies domains not on the configured allowlist
...
describe("Financial Policy Rules", () => {
  ...
  describe("financial.x402_domain_allowlist", () => {
    it("allows requests to an allowlisted domain", () => {
      const request = createRequest(
        mockX402Tool(),
        { url: "https://127.0.0.1/v1/resource" },
    ...
    it("denies requests to a lookalike domain that merely starts with the allowlisted host", () => {
      const request = createRequest(
        mockX402Tool(),
        { url: "https://127.0.0.1.evil.com/drain" },
        createMockSpendTracker(),
      );
    ...
    it("allows subdomain of an allowlisted domain", () => {
      const domainRules = createFinancialRules({
        ...DEFAULT_TREASURY_POLICY,
        x402AllowedDomains: ["example.com"],
      });
      const domainEngine = new PolicyEngine(db, domainRules);
      const request = createRequest(
        mockX402Tool(),
      ...
      const decision = domainEngine.evaluate(request);
      expect(decision.action).toBe("allow")
    });
