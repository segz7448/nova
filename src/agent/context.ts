...
/**
 * Above this length we skip the real BPE tokenizer entirely and fall back
 * to the character-based estimate. js-tiktoken's pure-JS encoder has
 * roughly quadratic (O(n^2)) blowup on long low-entropy runs (e.g. a
 * stuck tool repeating output, a giant log dump) — a single ~50k char
 * string of a repeated character can take 60+ seconds to encode, and a
 * 500k char one effectively hangs the process. That's not worth exact
 * token counts for context-budgeting purposes, where the /4 heuristic
 * is already "close enough" and used as the floor anyway.
 */
const MAX_TOKENIZE_LENGTH = 8_000;
...
export function estimateTokens(text: string): number {
  ...
  if (content.length > MAX_TOKENIZE_LENGTH) {
