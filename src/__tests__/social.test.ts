...
describe("Signing", () => {
  ...
  it("signSendPayload canonical format matches runtime and CLI expectation", async () => {
    ...
    const canonical = `Automaton:send:${to.toLowerCase()}:${contentHash}:${payload.signed_at}`;
...
describe("Relay URL Validation", () => {
  it("HTTPS URL accepted", async () => {
    ...
    expect(() => validateRelayUrl("https://social.example.com")).not.toThrow();
  });

  it("HTTP URL rejected", async () => {
    const { validateRelayUrl } = await import("../social/validation.js");
    expect(() => validateRelayUrl("http://social.example.com")).toThrow(
...
describe("Agent Card", () => {
  ...
  it("generateAgentCard does NOT include the backend API URL", async () => {
    const { generateAgentCard } = await import("../registry/agent-card.js");
    ...
    expect(cardStr).not.toContain("api.example.com");
  });
  ...
  it("hostAgentCard writes card as separate JSON file", async () => {
    ...
    await hostAgentCard(card, mockBackend);
