...
import { resolveMaxAgents } from "../../orchestration/types.js";
...
- If you have nothing left to delegate or verify, call task_done.`;
  ...
  private async refreshPlanInternal(params: {
    ...
    // buildPlannerContext needs the automaton's real current credit
    // balance to know what the planner is allowed to spend
    // (validatePlannerOutput rejects any plan estimated above it). This
    // call previously never supplied `creditsCents`, so it silently
    // fell back to planner-context.ts's cached-KV/zero default — an
    // orchestrator running fresh (empty cache) would have every real
    // plan rejected as "exceeding available credits (0)" regardless of
    // its actual balance. Fetch the live balance from the backend the
    // same way usdcBalance already is, falling back to the cache (via
    // leaving creditsCents undefined) only if the call fails.
    const creditsCents = await this.context.backend
      .getCreditsBalance()
      .then((cents) => (Number.isFinite(cents) ? cents : undefined))
      .catch(() => undefined);
    ...
    const plannerContext = await buildPlannerContext({
      ...
      maxAgents: this.context.config?.maxChildren === undefined
        ? Math.max(1, this.getManagedTasks().length || 1)
        : resolveMaxAgents(this.context.config.maxChildren),
    });
