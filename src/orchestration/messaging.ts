...
export class ColonyMessaging {
  ...
  /** Process only messages owned by this consumer. Unmatched rows remain
   * received for the agent loop (the single general inbox consumer). */
  async processInbox(types?: readonly MessageType[]): Promise<ProcessedMessage[]> {
