...
import { classifyFile, readScopeManifest } from "./merge-policy.js";
import { pushSelfModBranch } from "./sync.js";
import { finishWorkAndRestart } from "./lifecycle.js";
...
export async function editFile(
  ...
  let oldContent = "";
  try {
    oldContent = await backend.readFile(filePath);
  } catch {
    oldContent = "(new file)";
  }
  ...
  try {
    ...
    await gitCommit(backend, process.cwd(), `pre-modify: ${reason}`);
  } catch {
  ...
  // 7. Write new content
  try {
    await backend.writeFile(filePath, newContent);
  } catch (err: any) {
    return {
      success: false,
  ...
  try {
    const { gitCommit } = await import("../git/tools.js");
    await gitCommit(backend, process.cwd(), `self-mod: ${reason}`);
  } catch {
  ...
  if (/\.(ts|js|tsx|jsx)$/.test(filePath)) {
    try {
      await backend.exec("npm run build", 60_000);
    } catch {
      return { success: true, error: "File edited but rebuild failed. Run 'npm run build' manually." };
    }
  }

  // 11. Auto-push to this agent's OWN branch — never to `main`, and
  // never for a core (shared-engine) file. See merge-policy.ts for
  // why the split is at skills/: a core file is never something one
  // agent's unreviewed local edit should become a candidate branch
  // other agents' syncFromMain() might later evaluate — a stray core
  // self-mod (still technically possible here since core files aren't
  // all individually in PROTECTED_FILES) stays local-only, committed
  // by step 9 above but not pushed anywhere, pending explicit human
  // review. Only a scoped-owned edit — this agent's own actual
  // business area — is "the agent pushes its own fix," per this
  // repo's own design conversation.
  try {
    const manifest = await readScopeManifest(backend, process.cwd());
    const relativePath = path.relative(process.cwd(), path.resolve(filePath));
    if (classifyFile(relativePath, manifest) === "scoped-owned") {
      const address = db.getIdentity("address");
      if (address) {
        await pushSelfModBranch(backend, process.cwd(), address, reason);
      }
    }
  } catch {
    // Auto-push is a convenience on top of an already-successful,
    // already-committed edit — a push failure (e.g. no network, no
    // configured remote) must not turn a successful self-mod into a
    // reported failure.
  }

  // Drain this agent's departments/projects, then restart only this agent
  // when deployment supplies AGENT_RESTART_COMMAND. Never restart backend.
  await finishWorkAndRestart();
