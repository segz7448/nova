...
/**
 * Cheap non-cryptographic hash (FNV-1a) so the cache key doesn't embed
 * the full text content. The old version used `${model}::${text}` as
 * the literal Map key — for a large turn that meant hashing/comparing a
 * multi-hundred-KB string on every single cache lookup. Length is
 * included alongside the hash to keep collision risk low without
 * needing a stronger (slower) hash.
 */
...
export function formatCacheKey(text: string, model?: string): string {
  return `${model ?? "default"}::${hashText(text)}`;
}
