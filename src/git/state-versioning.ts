...
import type { BackendClient, AutomatonDatabase } from "../types.js";
...
export async function initStateRepo(
  ...
  const checkResult = await backend.exec(
...
`;
  ...
  await backend.writeFile(`${dir}/.gitignore`, gitignore);

  // Configure git user
  await backend.exec(
    `cd ${dir} && git config user.name "Automaton" && git config user.email "automaton@self-hosted.local"`,
    5000,
  );

  // Initial commit
  await gitCommit(backend, dir, "genesis: automaton state repository initialized");
}
...
export async function commitStateChange(
  ...
  const status = await gitStatus(backend, dir);
  if (status.clean) {
    return "No changes to commit";
  }

  const message = `${category}: ${description}`;
  const result = await gitCommit(backend, dir, message);
  return result;
}
...
export async function commitSoulUpdate(
  ...
  return commitStateChange(backend, description, "soul");
}
...
export async function commitHeartbeatChange(
  ...
  return commitStateChange(backend, description, "heartbeat");
}
...
export async function commitConfigChange(
  ...
  return commitStateChange(backend, description, "config");
}
...
export async function getStateHistory(
  ...
  const dir = resolveHome(AUTOMATON_DIR);
  return gitLog(backend, dir, limit);
}
