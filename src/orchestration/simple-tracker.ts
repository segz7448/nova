...
export class SimpleFundingProtocol implements FundingProtocol {
  ...
  async fundChild(childAddress: string, amountCents: number): Promise<{ success: boolean }> {
    ...
    try {
      const result = await this.backend.transferCredits(
        childAddress,
        transferAmount,
  ...
  // TODO: The backend API only exposes getCreditsBalance() for the calling agent's own
  ...
  // funding. When the backend API adds per-agent balance queries, replace this with
