...
describe("agent/CodingHarness confinement", () => {
  ...
  async function runTool(backend: BackendClient, toolName: string, args: Record<string, unknown>): Promise<string> {
  ...
  it("blocks patch_file traversal outside the allowed edit root", async () => {
    ...
    const out = await runTool(createBackendStub(), "patch_file", {
      path: outsideFile,
      search: "before",
      replace: "after",
    });
  ...
  it("blocks list_dir traversal outside the allowed edit root", async () => {
    const out = await runTool(createBackendStub(), "list_dir", { path: "../../etc" });
    expect(out).toContain("Blocked: path");
    expect(out).toContain("outside workspace");
  });
  ...
  it("allows patch_file inside the allowed edit root via local fallback", async () => {
    ...
    const out = await runTool(backend, "patch_file", {
      path: filePath,
      search: "'before'",
      replace: "'after'",
    });
