...
import type { ChildLifecycleState, BackendClient, ExecResult } from "../types.js";
...
describe("ChildHealthMonitor", () => {
  ...
  it("checkHealth returns healthy for running child", async () => {
    ...
    vi.spyOn(backend, "exec").mockResolvedValue({
      stdout: '{"status":"healthy","uptime":3600}',
      stderr: "",
      exitCode: 0,
    });

    const monitor = new ChildHealthMonitor(db, backend, lifecycle);
  ...
  it("checkHealth never throws, returns issues", async () => {
    ...
    vi.spyOn(backend, "exec").mockRejectedValue(new Error("sandbox unreachable"));
  ...
  it("checkAllChildren respects concurrency limit", async () => {
    ...
    vi.spyOn(backend, "exec").mockImplementation(async () => {
      concurrentCount++;
...
describe("SandboxCleanup", () => {
  ...
  it("cleanup only works on stopped/failed children", async () => {
    ...
    const cleanup = new SandboxCleanup(backend, lifecycle, db);
...
describe("Constitution", () => {
  ...
  it("propagateConstitution writes file and hash", async () => {
    ...
    await propagateConstitution(backend, "sandbox-1", db);
  ...
  it("verifyConstitution passes for matching hash", async () => {
    ...
    vi.spyOn(backend, "readFile").mockResolvedValue("We the automatons...");

    const result = await verifyConstitution(backend, "sandbox-1", db);
  ...
  it("verifyConstitution fails for tampered content", async () => {
    ...
    vi.spyOn(backend, "readFile").mockResolvedValue("We the EVIL automatons...");
