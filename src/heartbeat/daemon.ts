...
export function createHeartbeatDaemon(
  ...
  const { identity, config, heartbeatConfig, db, rawDb, backend, social, groups, onWakeRequest } = options;
  ...
  const forceRun = async (taskName: string): Promise<void> => {
    const context = await import("./tick-context.js").then((m) =>
      m.buildTickContext(rawDb, backend, heartbeatConfig, identity.address, identity.chainType),
    );
