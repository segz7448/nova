...
import { getUsdcBalance } from "../chain-utils/x402.js";
import {
  claimInboxMessages,
  markInboxProcessed,
  markInboxFailed,
...
export async function runAgentLoop(
  ...
  const { identity, config, db, backend, inference, social, groups, skills, policyEngine, spendTracker, onStateChange, onTurnComplete, ollamaBaseUrl } =
    options;

  const builtinTools = createBuiltinTools(identity.sandboxId);
  const builtinToolNames = new Set(builtinTools.map((t) => t.name));
  // `tools` is reloaded from the database at the top of every turn below
  // (see the `while (running)` loop) — NOT just once here — so a tool
  // registered via register_tool/install_mcp_server mid-session becomes
  // callable on the agent's very next turn without a process restart.
  // This initial value covers the first turn and the worker-pool wiring
  // just below, before the loop has run at all.
  let tools = [...builtinTools, ...loadInstalledTools(db, builtinToolNames)];
  ...
  if (hasTable(db.raw, "goals")) {
    try {
      ...
      // them from config.json or from SIWE provisioning against your backend.
      ...
      // Your backend's inference gateway is OpenAI-compatible. Use it as
      // fallback when no direct OpenAI key is available. The backendApiKey
      // is always present (required for sandbox operations), so this
      // ensures the orchestrator can always make inference calls.
      if (!process.env.OPENAI_API_KEY && config.backendApiKey) {
        process.env.OPENAI_API_KEY = config.backendApiKey;
        process.env.OPENAI_BASE_URL = `${config.backendApiUrl}/v1`;
      }
      ...
      // If OPENAI_BASE_URL was set (backend fallback), update the default
      // provider's baseUrl so the OpenAI client points to your backend's inference gateway.
      ...
      const funding = new SimpleFundingProtocol(backend, identity, db);
      ...
      // as async tasks. Falls back from sandbox spawning.
      ...
      orchestrator = new Orchestrator({
        ...
        config: {
          ...
          spawnAgent: async (task: any) => {
            // Try sandbox spawn first (production)
            try {
              ...
              const child = await spawnChild(backend, identity, db, genesis, lifecycle, groups);
              ...
              if (is402) {
                ...
                const chainIdentity = identity.chainIdentity;
                if (cooldownExpired && !chainIdentity) {
                  logger.warn("Sandbox topup skipped: no chain identity provisioned", {
                    taskId: task.id,
                  });
                } else if (cooldownExpired && chainIdentity) {
                  ...
                  try {
                    ...
                    if (topupResult?.success) {
                      ...
                      try {
                        ...
                        const child = await retrySpawn(backend, identity, db, retryGenesis, retryLifecycle);
                        return {
                          address: child.address,
                          name: child.name,
                          sandboxId: child.sandboxId,
              ...
              // Sandbox unavailable — fall back to local worker
              logger.info("Sandbox unavailable, spawning local worker", {
                taskId: task.id,
          ...
          startAgent: async (address: string) => {
            const child = db.getChildren().find((entry) => entry.address === address);
            if (!child) throw new Error(`Spawned child ${address} not found`);
            const { ChildLifecycle } = await import("../replication/lifecycle.js");
            const { startChildRuntime } = await import("../replication/spawn.js");
            const lifecycle = new ChildLifecycle(db.raw);
            if (child.status === "wallet_verified") {
              lifecycle.transition(child.id, "funded", "funded for orchestrated task");
            }
            await startChildRuntime(backend.createScopedClient(child.sandboxId), child.id, lifecycle);
          },
        },
      });
    } catch (error) {
  ...
  // Get financial state
  let financial = await getFinancialState(backend, identity.address, db, config.chainType || identity.chainType || "evm");
  ...
  while (running) {
    ...
    try {
      // Reload installed tools fresh every turn. This is the fix for
      // "agents can't register new tools at runtime": register_tool /
      // install_mcp_server / remove_tool only ever write to the
      // database — this is the one place that turns those writes back
      // into the live, callable `tools` array the rest of this turn
      // (system prompt, inference call, executeTool) actually uses. A
      // single indexed SELECT per turn; negligible cost.
      tools = [...builtinTools, ...loadInstalledTools(db, builtinToolNames)];
      ...
      financial = await getFinancialState(backend, identity.address, db, config.chainType || identity.chainType || "evm");
      ...
      if (financial.creditsCents === -1) {
        ...
        if ((tier === "critical" || tier === "low_compute") && financial.usdcBalance >= 5) {
          ...
          const chainIdentity = identity.chainIdentity;
          if (cooldownExpired && !chainIdentity) {
            logger.warn("Inline auto-topup skipped: no chain identity provisioned");
          } else if (cooldownExpired && chainIdentity) {
      ...
      const MUTATING_TOOLS = new Set([
        ...
        "install_npm_package", "install_mcp_server", "register_tool", "remove_tool", "install_skill",
...
async function getFinancialState(
  ...
  try {
    creditsCents = await backend.getCreditsBalance();
