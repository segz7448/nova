...
On first run, the runtime launches an interactive setup wizard — generates a wallet, registers against your own `automaton-stack` backend, asks for a name, genesis prompt, and creator address, then writes all config and starts the agent loop.

This runtime talks to your own self-hosted backend (see `../backend/`) — your VM, your wallet, your x402 facilitator. No third-party cloud account is required.
...
To help save Automatons Tokens & simplify setup of permissionless services & capabilities, we introduce Automaton Skills. We are open to contributions to make Automatons more capable.
...
A profitable automaton isn't limited to replicating itself — it can
also run a standing pipeline (Opportunity Intelligence → Research →
Finance → Strategy → Expansion Committee → CEO decision → Genesis)
that finds the highest-ROI adjacent opportunity and, on its own
approval, spawns an independently-missioned sibling company to pursue
it. This reuses the same wallet/sandbox/constitution machinery
described above for self-replication — the pipeline only decides when
and with what mission to call it. See
[`../backend/EXPANSION_PIPELINE.md`](../backend/EXPANSION_PIPELINE.md)
for the full reference, or `../Zent.md` for the build plan.
...
Automatons in this fork run on your own infrastructure: a VM you own, a self-hosted `automaton-stack` backend (see `../backend/`) for exec/wallet/x402, and your own inference provider. No third-party cloud account, and no dependency on any outside operator's infrastructure.
