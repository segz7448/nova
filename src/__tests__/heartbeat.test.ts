...
describe("Heartbeat Tasks", () => {
  ...
  describe("check_social_inbox", () => {
    ...
    it("polls and wakes when messages found", async () => {
      ...
      const envelope = JSON.parse(unprocessed[0].content);
      expect(envelope.protocol).toBe("colony_message_v1");
      expect(envelope.message.type).toBe("customer_request");
      // check_social_inbox routes raw message content through
      // injection-defense.ts's wrapSocialMessage (or equivalent), which
      // prefixes every social message with "[Message from <address>]:"
      // before persisting it — a trust-boundary marker so the agent's
      // prompt always distinguishes external/untrusted content from its
      // own system content (same defense-in-depth pattern already
      // covered for SOUL.md content in context-hardening.test.ts). The
      // raw message body is still present, just wrapped, not replaced.
      expect(envelope.message.content).toContain("[Message from 0xsender1]");
      expect(envelope.message.content).toContain("Hey there!");
      expect(envelope.message.from).toBe("0xsender1");
    });
  ...
  describe("revenue_opportunity_scan", () => {
    it("always wakes the sleeping agent to pursue productive work", async () => {
      const tickCtx = createMockTickContext(db);
      const taskCtx: HeartbeatLegacyContext = {
        identity: createTestIdentity(), config: createTestConfig(), db, backend,
      };
      const result = await BUILTIN_TASKS.revenue_opportunity_scan(tickCtx, taskCtx);
      expect(result.shouldWake).toBe(true);
      expect(result.message).toContain("opportunit");
    });
  });
  ...
  describe("health_check", () => {
    ...
    it("wakes when sandbox exec fails", async () => {
      backend.exec = async () => ({ stdout: "", stderr: "unhealthy", exitCode: 1 });
  ...
  describe("shared tick context", () => {
    it("all tasks receive the same tick context without redundant API calls", async () => {
      ...
      // (backend.getCreditsBalance is only called during buildTickContext, not by tasks)
    });
  });
});
