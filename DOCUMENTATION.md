...
An automaton is a continuously running, self-improving, self-replicating AI agent with its own Ethereum wallet and survival pressure. It runs inside a Linux VM sandbox or locally, pays for its own compute with USDC, and operates autonomously from the moment it boots.
...
Run `agent/scripts/automaton.sh` from a checkout of your own repo, or clone directly:
...
- Internet access (for your backend API, USDC on-chain operations)
...
The runtime signs a SIWE (Sign-In With Ethereum) message to authenticate with your backend and receive an API key. If auto-provisioning fails, you can enter a key manually.
...
The wizard detects whether you're running inside a managed sandbox (via environment variables) or locally. If a sandbox is detected, its ID is stored in the config.
...
2. **Transfer USDC directly** to the automaton's wallet address on-chain.
...
| `--provision` | Provision a backend API key via SIWE |
...
|---|---|
| `BACKEND_API_URL` | Your self-hosted backend URL (default: `http://127.0.0.1:8000`) |
| `BACKEND_API_KEY` | Your self-hosted backend's shared secret (overrides config file) |
...
- Fund via x402 payment to your backend's facilitator
...
Send USDC directly to the automaton's wallet address on-chain.

**3. Your own dashboard**

Fund via whatever wallet UI you use to manage the automaton's on-chain balance.
...
```jsonc
{
  ...
  // Infrastructure
  "sandboxId": "sbx_abc123",                    // Sandbox ID (empty = local mode)
  "backendApiUrl": "http://YOUR_VM_IP:8000",    // Your backend API endpoint
  ...
  "socialRelayUrl": "",  // point at your own self-hosted social relay, if you run one
  ...
  "treasuryPolicy": {
    ...
    "x402AllowedDomains": ["YOUR_VM_IP_OR_DOMAIN"],
...
When `sandboxId` is empty, the automaton runs in **local mode**: shell commands execute locally, file operations use the local filesystem. When set, operations route through your backend's sandbox API. On 403 errors (mismatched API key), the runtime falls back to local execution.
...
|---|---|---|
| `heartbeat_ping` | Every 15 min | Publishes status to your backend. Sends distress on critical/dead. |
...
| `refresh_models` | Configurable | Refreshes available models from your inference gateway. |
...
| Tool | Risk | Description |
|---|---|---|
| `check_credits` | safe | Check backend credit/wallet balance. |
...
| `heartbeat_ping` | safe | Publish status ping to your backend. |
...
| `x402AllowedDomains` | `[process.env.BACKEND_HOST \|\| "127.0.0.1"]` | Domains allowed for x402 |
...
1. Creates a new sandbox (VM) on your backend
...
Automatons communicate via a social relay — point `socialRelayUrl` at your own self-hosted relay.
...
1. **Backend proxy** (default) — routes through your own backend's inference gateway, billed from your wallet
...
Use the `list_models` tool to see current models with pricing. The model registry is refreshed from your inference gateway by the heartbeat.
...
- Verify your backend API's connectivity
...
**Can I run an automaton fully locally, without any cloud backend?**
...
**What happens if your backend API goes down?**
