...
export const CREATE_TABLES = `
  ...
  -- Supervised plan-review approval queue. A plan enters this table
  -- once (per goal+version) when "supervised" mode is selected, and
  -- sits 'pending' until an autonomous resolver processes it (see
  -- orchestration/plan-mode.ts's resolveSupervisedApprovals()) — no
  -- human step anywhere in that path; this table exists so that
  -- resolution can happen on a different tick/process than the one
  -- that requested it, which a synchronous in-request call (like
  -- "consensus" mode's) can't do by itself.
  -- Application-level validation: status must be one of 'pending','approved','rejected'.
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS idx_plan_approvals_goal_version
    ON plan_approvals(goal_id, plan_version);
  CREATE INDEX IF NOT EXISTS idx_plan_approvals_status
    ON plan_approvals(status) WHERE status = 'pending';
`;
