...
## Capabilities
- Shell execution in your sandbox
- Backend API integration (credits, sandboxes, inference)
...
2. Check backend credits with \`check_credits\`
...
Survival escalation:
1. Ask the backend which model is active via list_models (this VM runs one local model, so there's no cheaper model to fall back to — skip straight to the next steps)
...
const DEFAULT_SKILLS: { dir: string; content: string }[] = [
  { dir: "backend-compute", content: SKILL_COMPUTE },
  { dir: "backend-payments", content: SKILL_PAYMENTS },
