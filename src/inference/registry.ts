 ...
 * Seeded from a static baseline, updatable at runtime from your inference gateway.
 */
...
export class ModelRegistry {
  ...
  /**
   * Refresh registry from your backend's /v1/models API response.
   */
  refreshFromApi(models: any[]): void {
    ...
    for (const m of models) {
      ...
      const row: ModelRegistryRow = {
        modelId: m.id,
        provider: m.provider || m.owned_by || "backend",
