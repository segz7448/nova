...
import { logFlaggedAutoRevertsWithoutBackend } from "../self-mod/task-outcome.js";
import {
  assignTask,
  completeTask,
  decomposeGoal,
...
import { resolveMaxAgents } from "./types.js";
import { generateTodoMd } from "./attention.js";
import { UnifiedInferenceClient } from "../inference/inference-client.js";
import { reviewPlan, resolveSupervisedApprovals } from "./plan-mode.js";
...
export class Orchestrator {
  ...
  async tick(): Promise<OrchestratorTickResult> {
    ...
    // Drain any pending supervised-mode approval requests BEFORE
    // processing this tick's own phase — fully autonomous (see
    // resolveSupervisedApprovals()'s own doc comment): no human step
    // anywhere, just an independent critic call resolving whatever's
    // queued. A plan enqueued this tick becomes resolvable on the
    // very next one, decoupled from the request the same way a real
    // queue is meant to work.
    try {
      const resolution = await resolveSupervisedApprovals(this.params.db, this.params.inference);
      if (resolution.approved + resolution.rejected > 0) {
        logger.info("Resolved pending supervised plan approvals", resolution);
      }
    } catch (error) {
      // Belt-and-suspenders: resolveSupervisedApprovals() itself
      // never throws, but a queue drain failing must never block the
      // rest of this tick from running.
      logger.warn("Supervised approval resolution pass failed", { error: normalizeError(error).message });
    }
  ...
  async fundAgentForTask(addr: string, task: TaskNode): Promise<void> {
    ...
    const availableCents = await this.params.funding.getBalance(this.params.identity.address);
    if (!Number.isFinite(availableCents) || availableCents < amountCents) {
      throw new Error(
        `Insufficient available credits to fund task ${task.id}: required ${amountCents}, available ${Number.isFinite(availableCents) ? availableCents : "unknown"}`,
      );
    }
  ...
  async collectResults(): Promise<TaskResult[]> {
    ...
    // Task results are the only inbox messages owned by the orchestrator.
    // All other messages must remain for the normal agent loop to claim.
    const processed = await this.params.messaging.processInbox(["task_result"]);
    for (const entry of processed) {
      ...
      const task = getTaskById(this.params.db, parsed.taskId);
      const authorized = task
        && task.goalId === parsed.goalId
        && task.assignedTo !== null
        && task.assignedTo === entry.message.from
        && (task.status === "assigned" || task.status === "running");
      if (!authorized) {
        logger.warn("Rejected unauthorized task result", {
  ...
  async handleFailure(task: TaskNode, error: string): Promise<void> {
    const flaggedForRevert = failTask(this.params.db, task.id, error, true);
    if (flaggedForRevert.length > 0) {
      logFlaggedAutoRevertsWithoutBackend(flaggedForRevert, logger);
    }
    ...
    try {
      output = await planGoal(
        ...
        await buildPlannerContext({
          ...
          maxAgents: resolveMaxAgents(this.params.config?.maxChildren),
        }),
  ...
  private async handlePlanReviewPhase(state: OrchestratorState): Promise<OrchestratorState> {
    ...
    try {
      const result = await reviewPlan(
        planData as any,
        {
          mode: (this.params.config?.planApprovalMode as any) ?? "auto",
          autoBudgetThreshold: this.params.config?.planApprovalAutoBudgetThresholdCents ?? 5000,
          consensusCriticRole: this.params.config?.planApprovalConsensusCriticRole ?? "reviewer",
          reviewTimeoutMs: this.params.config?.planApprovalReviewTimeoutMs ?? 1800000,
        },
        this.params.inference,
        { db: this.params.db, goalId: state.goalId, planVersion: state.replanCount },
      );
      ...
      const feedbackWithConcerns =
        result.concerns && result.concerns.length > 0
          ? `${result.feedback ?? "Plan rejected"}\nConcerns:\n${result.concerns.map((c) => `- ${c}`).join("\n")}`
          : result.feedback ?? "Plan rejected";

      this.params.db.prepare(
        "INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))",
      ).run(`orchestrator.review_feedback.${state.goalId}`, feedbackWithConcerns);

      return { ...state, phase: "planning" };
    } catch (error) {
      const err = normalizeError(error);
      if (err.message === "awaiting supervised approval") {
        return state;
      }
      throw error;
    }
  ...
  private async handleExecutingPhase(
    ...
    for (const task of ready) {
      try {
        ...
        if (!isLocalWorker && !isSelfAssigned) {
          ...
          if (assignment.spawned && typeof this.params.config?.startAgent === "function") {
            await this.params.config.startAgent(assignment.agentAddress);
          }
    ...
    for (const event of this.pendingTaskResults) {
      ...
      if (event.result.success) {
        try {
          const flaggedForRevert = completeTask(this.params.db, taskRow.id, event.result);
...
function parseTaskResultMessage(message: AgentMessage): TaskResultEnvelope | null {
  ...
  return {
    taskId,
    goalId: firstString(obj.goalId, message.goalId),
    result,
