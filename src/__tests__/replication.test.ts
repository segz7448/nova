 ...
 *
 * Updated for Phase 3.1: spawnChild now uses BackendClient interface
...
// Mock the repo-URL resolver spawn.ts uses for provisioning child
// sandboxes — tests shouldn't depend on this checkout's real git state
// (origin remote may or may not be configured in CI/sandbox environments).
vi.mock("../self-mod/upstream.js", () => ({
  getRuntimeRepoUrl: vi.fn(() => "https://example.com/test/automaton.git"),
}));
...
describe("spawnChild", () => {
  ...
  it("validates wallet address before creating child record", async () => {
    ...
    vi.spyOn(backend, "exec").mockImplementation(async (command: string) => {
    ...
    const child = await spawnChild(backend, identity, db, genesis);
  ...
  it("throws on zero address from init", async () => {
    ...
    await expect(spawnChild(backend, identity, db, genesis))
  ...
  it("propagates error on exec failure without calling deleteSandbox", async () => {
    const deleteSpy = vi.spyOn(backend, "deleteSandbox");

    // Make the first exec (apt-get install) fail
    vi.spyOn(backend, "exec").mockRejectedValue(new Error("Install failed"));
  ...
  it("does not mask original error if deleteSandbox also throws", async () => {
    vi.spyOn(backend, "deleteSandbox").mockRejectedValue(new Error("delete also failed"));
  ...
  it("does not call deleteSandbox if createSandbox itself fails", async () => {
    const deleteSpy = vi.spyOn(backend, "deleteSandbox");
    vi.spyOn(backend, "createSandbox").mockRejectedValue(new Error("Sandbox creation failed"));
...
describe("SandboxCleanup", () => {
  ...
  it("transitions to cleaned_up even though sandbox deletion is disabled", async () => {
    ...
    const cleanup = new SandboxCleanup(backend, lifecycle, db.raw);
