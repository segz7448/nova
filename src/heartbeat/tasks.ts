...
import { getSurvivalTier } from "../chain-utils/credits.js";
...
import { runConstitutionIntegrityCheck } from "../soul/constitution-guard.js";
...
export const COLONY_TASK_INTERVALS_MS = {
  ...
  // Deliberately the shortest interval of any built-in task. This is
  // the one check where "found out an hour late" is not an acceptable
  // trade-off for lower overhead — see constitution-guard.ts.
  constitution_integrity_check: 120_000,
  revenue_opportunity_scan: 900_000,
} as const;

export const BUILTIN_TASKS: Record<string, HeartbeatTaskFn> = {
  // Must never be shed under low_compute (see checkKillSwitch.ts's own
  // header comment) — if you introduce a task-shedding policy keyed off
  // survival tier, exclude this key from it explicitly.
  check_kill_switch: checkKillSwitch,
  heartbeat_ping: async (ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    // Use ctx.creditBalance instead of calling backend.getCreditsBalance()
  ...
  revenue_opportunity_scan: async (ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    const timestamp = new Date().toISOString();
    taskCtx.db.setKV("last_revenue_opportunity_scan", JSON.stringify({
      timestamp,
    ...
    return {
      shouldWake: true,
      message: "Continuous-work wake: inspect the market, identify a feasible revenue opportunity, and execute the next validated step.",
    };
  },
  ...
  check_usdc_balance: async (ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    ...
    if (balance >= MIN_TOPUP_USD && (ctx.survivalTier === "critical" || ctx.survivalTier === "dead")) {
      ...
      const chainIdentity = taskCtx.identity.chainIdentity;
      if (!chainIdentity) {
        // Same situation as a failed topup attempt below: we can't
        // auto-resolve this ourselves, so the agent needs to be woken
        // to handle it manually (e.g. provision a chain identity, or
        // otherwise move funds). Silently returning shouldWake: false
        // here would strand an automaton that has USDC on hand but no
        // signer — it would never be told anything is wrong.
        logger.warn("Auto-topup skipped: no chain identity provisioned");
        return {
          shouldWake: true,
          message: `Low credits ($${(credits / 100).toFixed(2)}) with USDC available ($${balance.toFixed(2)}) but no chain identity is provisioned, so auto-topup can't run. Provision a chain identity or use topup_credits to resolve manually.`,
        };
      }

      taskCtx.db.setKV("last_auto_topup_attempt", new Date().toISOString());

      const { bootstrapTopup } = await import("../chain-utils/topup.js");
      const result = await bootstrapTopup({
        apiUrl: taskCtx.config.backendApiUrl,
  ...
  check_social_inbox: async (_ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    ...
    for (const msg of messages) {
      ...
      if (!existing) {
        ...
        const sanitizedTo = sanitizeInput(msg.to, msg.to, "social_address");
  ...
  refresh_models: async (_ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    try {
      const models = await taskCtx.backend.listModels();
      if (models.length > 0) {
  ...
  check_child_health: async (_ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    try {
      ...
      const monitor = new ChildHealthMonitor(taskCtx.db.raw, taskCtx.backend, lifecycle);
  ...
  prune_dead_children: async (_ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    try {
      ...
      const lifecycle = new ChildLifecycle(taskCtx.db.raw);
      const cleanup = new SandboxCleanup(taskCtx.backend, lifecycle, taskCtx.db.raw);
  ...
  health_check: async (_ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    ...
    try {
      const result = await taskCtx.backend.exec("echo alive", 5000);
  ...
  agent_pool_optimize: async (_ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    ...
    try {
      ...
      const spawnCapacity = taskCtx.config.maxChildren < 0
        ? spawnNeeded // unlimited: capacity is never the limiting factor
        : Math.max(0, taskCtx.config.maxChildren - activeAgents);
  ...
  // Re-hashes the live constitution.md against the hash recorded at
  // genesis on every tick. This runs whether or not the agent is awake
  // — the whole point is to catch tampering that happened through a
  // path other than the agent's own tools (see constitution-guard.ts),
  // so it can't depend on the agent loop being the thing that notices.
  //
  // On mismatch: latches a sticky "compromised" flag in the KV store,
  // writes an unreversible audit entry, and wakes the agent with an
  // urgent message. constitution-integrity.ts (a priority-0 policy
  // rule) reads that same flag on every subsequent tool-call evaluation
  // and denies everything until an operator clears it — so the halt
  // takes effect before this tick even finishes, not after the agent
  // "decides" to respect it.
  constitution_integrity_check: async (_ctx: TickContext, taskCtx: HeartbeatLegacyContext) => {
    if (!shouldRunAtInterval(taskCtx, "constitution_integrity_check", COLONY_TASK_INTERVALS_MS.constitution_integrity_check)) {
      return { shouldWake: false };
    }

    try {
      const result = runConstitutionIntegrityCheck(taskCtx.db);

      if (!result.ok) {
        logger.error(`CONSTITUTION INTEGRITY VIOLATION: ${result.detail}`);
        return {
          shouldWake: true,
          message:
            `URGENT: constitution.md integrity check failed (${result.detail}). ` +
            `All tool calls are now denied until a human operator investigates and clears the flag. ` +
            `Do not attempt to work around this — report your status and wait.`,
        };
      }

      return { shouldWake: false };
    } catch (error) {
      // A failure to even run the check is itself worth surfacing loudly
      // rather than silently skipping — unlike the other tasks above,
      // this one fails closed conceptually even though it can't set the
      // compromised flag from inside a catch block cleanly here (the
      // check function already does its own try/catch internally for
      // the file read; this outer catch is for truly unexpected errors).
      logger.error("constitution_integrity_check crashed", error instanceof Error ? error : undefined);
      return { shouldWake: false };
    }
  },
};
...
async function createHealthMonitor(taskCtx: HeartbeatLegacyContext): Promise<ColonyHealthMonitor> {
  ...
  const funding = new SimpleFundingProtocol(taskCtx.backend, taskCtx.identity, taskCtx.db);
