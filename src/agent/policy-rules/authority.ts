...
const EXTERNAL_BLOCKED_TOOLS = [
  ...
  // Dynamic tool registration executes real code (a spawned MCP server
  // process, or an arbitrary shell command) the very next turn it's
  // called from any input source — this must never be reachable from
  // external/heartbeat-triggered input, same reasoning as spawn_child.
  "install_mcp_server",
  "register_tool",
  "install_npm_package",
] as const;
