 ...
 * - pull_upstream uses backend.exec() not host execSync
...
/**
 * Minimal mock of AutomatonDatabase — enough for every policy rule in
 * createDefaultRules() to evaluate without throwing. Only `getKV` is
 * actually read (by constitution-integrity's isConstitutionCompromised,
 * and defensively by rate-limits via optional chaining). Always reports
 * "not compromised" so these injection tests exercise the rule they're
 * actually named for (shell_injection / git_hash validation) instead of
 * being short-circuited by the constitution guard.
 */
function makeMockDb(): AutomatonDatabase {
  return {
    getKV: () => undefined,
    setKV: () => {},
...
function makeRequest(
  ...
  return {
    ...
    args,
    context: { db: makeMockDb() } as ToolContext,
    turnContext: {
      inputSource: "agent",
      turnToolCallCount: 0,
...
describe("command.forbidden_patterns rule", () => {
  ...
  const protectedWriteVariants = [
    "printf x > constitution.md",
    "printf x | tee agent/tools.ts",
    "cp /tmp/new constitution.md",
    "mv /tmp/new policy-engine.ts",
    "python -c \"open('wallet.json','w').write('x')\"",
    "node -e \"require('fs').writeFileSync('skills/loader.ts','x')\"",
  ];

  for (const cmd of protectedWriteVariants) {
    it(`blocks protected shell write: ${cmd}`, () => {
      const request = makeRequest("exec", { command: cmd }, "vm", "caution");
      const result = forbiddenRule.evaluate(request);
      expect(result).not.toBeNull();
      expect(result!.action).toBe("deny");
...
describe("Source code injection safety", () => {
  ...
  it("registry.ts uses execFileSync not backend.exec with interpolation", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync(
      new URL("../skills/registry.ts", import.meta.url).pathname.replace("/src/__tests__/../", "/src/"),
      "utf-8",
    );
    // Should NOT have template literals in backend.exec calls
    expect(source).not.toMatch(/backend\.exec\s*\(\s*`/);
  ...
  it("tools.ts pull_upstream uses backend.exec not host execSync", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync(
    ...
    expect(pullSection).toMatch(/ctx\.backend\.exec\(/);
  });
