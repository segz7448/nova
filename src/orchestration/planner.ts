...
export async function planGoal(
  ...
  return runPlannerInference({
    ...
    context,
    inference,
    availableCreditsCents: context.creditsCents,
  });
}
...
function runPlannerInference(params: {
  ...
  }).then((result) => validatePlannerOutput(parsePlannerResponse(result.content), params.availableCreditsCents));
}
...
<identity>
You are the strategic planner for an autonomous agent colony running on
your own self-hosted infrastructure. You are the colony's chief strategist, project decomposer, and
...
- **Integration agent**: Bridges two systems (e.g., "stripe-backend-bridge",
...
export function validatePlannerOutput(output: unknown, availableCreditsCents?: number): PlannerOutput {
  ...
  const taskCostCents = tasks.reduce((total, task) => total + task.estimatedCostCents, 0);
  if (estimatedTotalCostCents < taskCostCents) {
    throw new Error(
      `estimatedTotalCostCents (${estimatedTotalCostCents}) is below the sum of task estimates (${taskCostCents})`,
    );
  }
  if (availableCreditsCents !== undefined) {
    if (!Number.isFinite(availableCreditsCents) || availableCreditsCents < 0) {
      throw new Error("available credits must be a finite non-negative number");
    }
    if (estimatedTotalCostCents > availableCreditsCents) {
      throw new Error(
        `estimatedTotalCostCents (${estimatedTotalCostCents}) exceeds available credits (${availableCreditsCents})`,
      );
    }
  }
