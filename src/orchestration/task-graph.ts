...
import { recordTaskOutcomeForActiveSkills } from "../self-mod/task-outcome.js";
...
export function completeTask(db: Database, taskId: string, result: TaskResult): string[] {
  ...
  withTransaction(db, () => {
    ...
    // Best-effort self-mod scoring — see task-outcome.ts's own doc
    // comment for why this attributes credit to every currently
    // active scoped-owned skill file rather than one specific file.
    flaggedForRevert = recordTaskOutcomeForActiveSkills(db, process.cwd(), true);
  });
  return flaggedForRevert;
}

export function failTask(db: Database, taskId: string, error: string, shouldRetry: boolean): string[] {
  let flaggedForRevert: string[] = [];
  withTransaction(db, () => {
    const task = getTaskById(db, taskId);
    if (!task) {
    ...
    if (canRetry) {
      ...
      flaggedForRevert = recordTaskOutcomeForActiveSkills(db, process.cwd(), false, error);
      return;
    }
