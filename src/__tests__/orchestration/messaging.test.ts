...
describe("orchestration/messaging", () => {
  ...
  describe("ColonyMessaging.processInbox", () => {
    it("leaves non-result messages unprocessed when collecting task results", async () => {
      const ctx = createTestDb();
      raw = ctx.raw;
      const messaging = new ColonyMessaging({ deliver: vi.fn(), getRecipients: () => [] }, ctx.db);
      insertInbox(raw, { id: "agent-alert", from: "0xfrom", content: JSON.stringify(makeMessage({ type: "alert" })) });

      await messaging.processInbox(["task_result"]);

      const row = raw.prepare("SELECT processed_at FROM inbox_messages WHERE id = 'agent-alert'").get() as { processed_at: string | null };
