...
describe("Agent Loop", () => {
  ...
  it("exec tool runs and is persisted", async () => {
    ...
    // Verify backend.exec was called
    expect(backend.execCalls.length).toBeGreaterThanOrEqual(1);
    expect(backend.execCalls[0].command).toBe("echo hello");
  });
  ...
  it("forbidden patterns blocked", async () => {
    ...
    // backend.exec should NOT have been called
    expect(backend.execCalls.length).toBe(0);
  });

  it("low credits forces low-compute mode", async () => {
    backend.creditsCents = 50; // Below $1 threshold -> critical
  ...
  it("financial state cached fallback on API failure", async () => {
    ...
    backend.getCreditsBalance = async () => {
  ...
  it("zero credits enters critical tier, not dead", async () => {
    backend.creditsCents = 0; // $0 -> critical tier (agent stays alive)
  ...
  it("read_file turns are retained in context (not classified as idle)", async () => {
    backend.files["/tmp/one.txt"] = "one";
    backend.files["/tmp/two.txt"] = "two";
    backend.files["/tmp/three.txt"] = "three";
