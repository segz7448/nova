...
// Mock the MCP client pool so install_mcp_server tests never spawn a
// real process — these tests only exercise our own input validation.
const mcpListToolsMock = vi.fn(async () => [{ name: "noop", description: "noop" }]);
vi.mock("../mcp/client-pool.js", () => ({
  listMcpTools: (...args: unknown[]) => mcpListToolsMock(...args),
...
describe("read_file sensitive file blocking", () => {
  ...
  it("allows reading safe files", async () => {
    const readTool = tools.find((t) => t.name === "read_file")!;
    backend.files["/home/automaton/README.md"] = "# Hello";
...
describe("read_file fallback shell escaping", () => {
  ...
  it("escapes shell metacharacters in fallback cat command", async () => {
    ...
    vi.spyOn(backend, "readFile").mockRejectedValue(new Error("API broken"));

    await readTool.execute({ path: "/home/user/my file.txt" }, ctx);

    expect(backend.execCalls.length).toBe(1);
    // The path should be wrapped in single quotes by escapeShellArg
    expect(backend.execCalls[0].command).toBe("cat '/home/user/my file.txt'");
  });
  ...
  it("prevents command injection via semicolons in fallback path", async () => {
    ...
    expect(backend.execCalls[0].command).toBe("cat 'foo; cat /etc/passwd'");
  });
  ...
  it("escapes single quotes in file path in fallback", async () => {
    ...
    expect(backend.execCalls[0].command).toBe("cat 'it'\\''s a file.txt'");
  });
  ...
  it("prevents subshell injection via $() in fallback path", async () => {
    ...
    expect(backend.execCalls[0].command).toBe("cat '$(whoami).txt'");
  });
});
...
describe("exec tool forbidden command patterns", () => {
  ...
  for (const cmd of FORBIDDEN_COMMANDS) {
    it(`blocks: ${cmd.slice(0, 60)}`, async () => {
      ...
      expect(result).toContain("Blocked");
      expect(backend.execCalls.length).toBe(0);
    });
  }
...
describe("transfer_credits self-preservation", () => {
  ...
  beforeEach(() => {
    ...
    db = createTestDb();
    backend = new MockBackendClient();
    backend.creditsCents = 10_000; // $100
    ctx = {
      identity: createTestIdentity(),
      config: createTestConfig(),
      db,
...
describe("package install inline validation", () => {
  ...
  // install_mcp_server no longer shells out to `npm install` at all — it
  // spawns the server's own command directly (argv, no shell), so a
  // string like "pkg; rm -rf /" is just an invalid/nonexistent program
  // name, not a shell-injection vector. What still needs validating is
  // the server `name` (used to namespace its tools and stored in the
  // database) — same malicious strings, now as the `name` field.
  ...
  for (const name of MALICIOUS_NAMES) {
    it(`install_mcp_server rejects malicious server name: ${name.slice(0, 40)}`, async () => {
      const tool = tools.find((t) => t.name === "install_mcp_server")!;
      const result = await tool.execute({ name, command: "npx" }, ctx);
      expect(result).toContain("Failed to install MCP server");
      // Validation must happen before ever attempting to connect.
      expect(mcpListToolsMock).not.toHaveBeenCalled();
    });
  }

  it("install_mcp_server accepts a clean server name and registers its discovered tools", async () => {
    const tool = tools.find((t) => t.name === "install_mcp_server")!;
    const result = await tool.execute(
      { name: "search_server", command: "npx", args: ["-y", "@some/mcp"] },
      ctx,
    );
    expect(result).toContain("installed with 1 tool(s)");
    expect(mcpListToolsMock).toHaveBeenCalled();
  });
  ...
  it("install_npm_package allows clean package names", async () => {
    ...
    expect(backend.execCalls.length).toBe(1);
    expect(backend.execCalls[0].command).toBe("npm install -g axios");
  });

  it("install_npm_package allows scoped packages", async () => {
    const tool = tools.find((t) => t.name === "install_npm_package")!;
    await tool.execute({ package: "@backend/automaton" }, ctx);
    expect(backend.execCalls.length).toBe(1);
  });
});
