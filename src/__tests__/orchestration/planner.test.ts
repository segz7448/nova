...
describe("validatePlannerOutput", () => {
  ...
  it("detects dependency cycle (A depends on B depends on A)", () => {
    const output = validOutput({
      ...
      // validOutput()'s default estimatedTotalCostCents (100) is below
      // this test's own two 100-cent tasks (200 total), which trips the
      // budget-consistency check before validation ever reaches cycle
      // detection (validatePlannerOutput checks budget first, then
      // dependencies — see planner.ts). Override it so this test
      // actually exercises the cycle check it's named for.
...
describe("custom role validation", () => {
  ...
  it("rejects a plan whose estimated cost exceeds available credits", () => {
    expect(() => validatePlannerOutput(validOutput({ estimatedTotalCostCents: 10001 }), 10000))
      .toThrow("available credits");
  });

  it("accepts a plan exactly equal to available credits", () => {
    expect(validatePlannerOutput(validOutput({ estimatedTotalCostCents: 10000 }), 10000)
      .estimatedTotalCostCents).toBe(10000);
  });
});
