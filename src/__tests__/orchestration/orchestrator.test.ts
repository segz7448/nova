...
import { getApprovalRequest } from "../../orchestration/plan-mode.js";
...
describe("orchestration/Orchestrator", () => {
  ...
  describe("collectResults", () => {
    function buildMessagingWithResults(
      ...
      messages: Array<{ type: string; content: string; goalId?: string; from?: string }>,
    ) {
    ...
    it("processes task_result messages from inbox", async () => {
      const goalId = insertGoal(db);
      const taskId = insertTask(db, { goalId, status: "running", assignedTo: "0xagent" });
      const content = JSON.stringify({ taskId, success: true, output: "done", artifacts: [], costCents: 5, duration: 100 });
      const messaging = buildMessagingWithResults(db, [{ type: "task_result", content, goalId, from: "0xagent" }]);
    ...
    it("rejects task results from an unassigned sender", async () => {
      const goalId = insertGoal(db);
      const taskId = insertTask(db, { goalId, status: "running", assignedTo: "0xagent" });
      const content = JSON.stringify({ taskId, success: true, output: "spoofed" });
      const messaging = buildMessagingWithResults(db, [{ type: "task_result", content, goalId, from: "0xattacker" }]);
      const orc = makeOrchestrator(db, { messaging });
      expect(await orc.collectResults()).toHaveLength(0);
    });
  ...
  describe("handlePlanReviewPhase (via tick)", () => {
    ...
    it("supervised mode (now actually configurable, not hardcoded) enqueues a real approval request and waits", async () => {
      const goalId = insertGoal(db);
      insertTask(db, { goalId, title: "t1", description: "desc" });
      storePlan(db, goalId, { estimatedTotalCostCents: 100 });
      setOrchestratorState(db, { phase: "plan_review", goalId, replanCount: 0, failedTaskId: null, failedError: null });

      const orc = makeOrchestrator(db, { config: { planApprovalMode: "supervised" } });
      const result = await orc.tick();

      expect(result.phase).toBe("plan_review"); // genuinely queued, not yet resolved — this tick's own request
      const request = getApprovalRequest(db, goalId, 0);
      expect(request?.status).toBe("pending");
    });

    it("supervised mode: fully autonomous end-to-end — a later tick's resolver pass approves the queued plan with zero human involvement, and the NEXT tick after that reaches executing", async () => {
      const goalId = insertGoal(db);
      insertTask(db, { goalId, title: "t1", description: "desc" });
      storePlan(db, goalId, { estimatedTotalCostCents: 100 });
      setOrchestratorState(db, { phase: "plan_review", goalId, replanCount: 0, failedTaskId: null, failedError: null });
      ...
      const fakeInference = {
        chat: vi.fn().mockResolvedValue({
          content: JSON.stringify({ approved: true, concerns: [], feedback: "Autonomous supervisor approves." }),
          usage: { inputTokens: 10, outputTokens: 10 },
        }),
      };
      const orc = makeOrchestrator(db, {
        config: { planApprovalMode: "supervised" },
        inference: fakeInference as any,
      });

      // Tick 1: enqueues the request, still pending, stays in plan_review.
      const tick1 = await orc.tick();
      expect(tick1.phase).toBe("plan_review");
      expect(getApprovalRequest(db, goalId, 0)?.status).toBe("pending");

      // Tick 2: the SAME orchestrator's own resolver pass (top of
      // tick()) drains the queue autonomously before this tick's own
      // phase handling even runs — no human touched anything between
      // tick 1 and tick 2.
      const tick2 = await orc.tick();
      expect(tick2.phase).toBe("executing");
      expect(getApprovalRequest(db, goalId, 0)?.status).toBe("approved");
      expect(fakeInference.chat).toHaveBeenCalledTimes(1);
    });

    it("supervised mode: a rejecting resolver pass sends the goal back to planning, still with zero human involvement", async () => {
      const goalId = insertGoal(db);
      insertTask(db, { goalId, title: "t1", description: "desc" });
      storePlan(db, goalId, { estimatedTotalCostCents: 100 });
      setOrchestratorState(db, { phase: "plan_review", goalId, replanCount: 0, failedTaskId: null, failedError: null });
      ...
      const fakeInference = {
        chat: vi.fn().mockResolvedValue({
          content: JSON.stringify({
            approved: false,
            concerns: ["Plan doesn't cover the stated compliance risk"],
            feedback: "Rejected: risk coverage gap.",
          }),
          usage: { inputTokens: 10, outputTokens: 10 },
        }),
      };
      ...
      await orc.tick(); // enqueue
      const tick2 = await orc.tick(); // resolver rejects it

      expect(tick2.phase).toBe("planning");
      expect(getApprovalRequest(db, goalId, 0)?.status).toBe("rejected");
      const feedbackRow = db
        .prepare("SELECT value FROM kv WHERE key = ?")
        .get(`orchestrator.review_feedback.${goalId}`) as { value: string } | undefined;
      expect(feedbackRow?.value).toContain("risk coverage gap");
      expect(feedbackRow?.value).toContain("compliance risk");
    });

    it("supervised mode: a transient resolver failure leaves the plan pending (not silently rejected or approved), and a later tick recovers", async () => {
      const goalId = insertGoal(db);
      insertTask(db, { goalId, title: "t1", description: "desc" });
      storePlan(db, goalId, { estimatedTotalCostCents: 100 });
      setOrchestratorState(db, { phase: "plan_review", goalId, replanCount: 0, failedTaskId: null, failedError: null });
      ...
      const fakeInference = {
        chat: vi.fn().mockImplementation(async () => {
          ...
          return {
            content: JSON.stringify({ approved: true, concerns: [], feedback: "Recovered." }),
            usage: { inputTokens: 10, outputTokens: 10 },
          };
        }),
      };
      ...
      await orc.tick(); // enqueue
      const tick2 = await orc.tick(); // resolver's critic call fails — must not crash the tick or reject the plan
      expect(tick2.phase).toBe("plan_review");
      expect(getApprovalRequest(db, goalId, 0)?.status).toBe("pending");

      const tick3 = await orc.tick(); // resolver succeeds this time
      expect(tick3.phase).toBe("executing");
    });

    it("consensus mode: a real approving critic (via the real inference client) transitions to executing", async () => {
      const goalId = insertGoal(db);
      insertTask(db, { goalId, title: "t1", description: "desc" });
      storePlan(db, goalId, { estimatedTotalCostCents: 100 });
      setOrchestratorState(db, { phase: "plan_review", goalId, replanCount: 0, failedTaskId: null, failedError: null });
      ...
      const fakeInference = {
        chat: vi.fn().mockResolvedValue({
          content: JSON.stringify({ approved: true, concerns: [], feedback: "Plan looks sound." }),
          usage: { inputTokens: 10, outputTokens: 10 },
        }),
      };

      const orc = makeOrchestrator(db, {
        config: { planApprovalMode: "consensus", planApprovalConsensusCriticRole: "risk-analyst" },
        inference: fakeInference as any,
      });
      const result = await orc.tick();

      expect(result.phase).toBe("executing");
      expect(fakeInference.chat).toHaveBeenCalled();
    });

    it("consensus mode: a real rejecting critic transitions back to planning and stores concerns in kv", async () => {
      const goalId = insertGoal(db);
      insertTask(db, { goalId, title: "t1", description: "desc" });
      storePlan(db, goalId, { estimatedTotalCostCents: 100 });
      setOrchestratorState(db, { phase: "plan_review", goalId, replanCount: 0, failedTaskId: null, failedError: null });
      ...
      const fakeInference = {
        chat: vi.fn().mockResolvedValue({
          content: JSON.stringify({
            approved: false,
            concerns: ["Task list doesn't cover the stated compliance risk"],
            feedback: "Plan has a risk-coverage gap.",
          }),
          usage: { inputTokens: 10, outputTokens: 10 },
        }),
      };
      ...
      const orc = makeOrchestrator(db, {
        config: { planApprovalMode: "consensus" },
        inference: fakeInference as any,
      });
      const result = await orc.tick();
    ...
    it("consensus mode with no working critic (inference throws) fails CLOSED — plan does NOT reach executing", async () => {
      const goalId = insertGoal(db);
      insertTask(db, { goalId, title: "t1", description: "desc" });
      storePlan(db, goalId, { estimatedTotalCostCents: 100 });
      setOrchestratorState(db, { phase: "plan_review", goalId, replanCount: 0, failedTaskId: null, failedError: null });
      ...
      // The old stub's bug, made concrete: it would have reached "executing"
      // with zero real review. It must NOT do that now.
      expect(result.phase).not.toBe("executing");
      expect(result.phase).toBe("planning");
    });
  });
});
