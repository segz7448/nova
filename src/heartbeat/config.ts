...
const DEFAULT_HEARTBEAT_CONFIG: HeartbeatConfig = {
  entries: [
    {
      // Must run more often than anything else here and must never be
      // disabled or shed under low_compute — see checkKillSwitch.ts's
      // own header comment for why. Every minute is cron's tightest
      // granularity; this is the one task where "as often as possible"
      // is the correct answer, not a performance tradeoff to tune.
    ...
    {
      name: "revenue_opportunity_scan",
      schedule: "*/15 * * * *",
      task: "revenue_opportunity_scan",
      enabled: true,
    },
  ],
